/**
 * Canvas UI "ASCII Sweep", vendored from https://canvasui.dev/docs and stripped
 * to its framework-agnostic core. The upstream file ships a React component
 * wrapped around createAsciiSweep(); only the wrapper is React, and the engine
 * below has no imports at all, so this site keeps shipping zero framework.
 *
 * Two code paths:
 *   - html-in-canvas (Chrome with #canvas-draw-element): the browser rasterises
 *     the live DOM subtree into the source canvas via drawElementImage().
 *   - fallback: paintFallbackSnapshot() walks the subtree and repaints text,
 *     borders, backgrounds and media onto a 2D canvas by hand.
 * The engine picks whichever is available, so this works in every browser.
 *
 * The effect sweeps between two panels. Give both slots the same content and it
 * reads as a one-shot reveal over that content instead of a swap.
 */
...
export interface AsciiSweepOptions {
  ...
  invert?: number;
  /** Contrast against the background above which a pixel counts as content. */
  threshold?: number;
  /** How far the content dims underneath the band (0 to 1). */
  fade?: number;
  /** How characters composite over the content. */
  blend?: AsciiSweepBlend;
  /** Color of the page behind the content, or "auto" to read it from the DOM. */
  background?: string;
...
export interface AsciiSweepInstance {
  ...
  /** Sweep to slot 0 or 1. Pass an angle to override the configured direction. */
...
function intersectFallbackRects(first: FallbackRect, second: FallbackRect): FallbackRect {
  return {
...
/** Above this many characters, per-character line measurement isn't worth it. */
const PER_CHAR_SPLIT_LIMIT = 600;

/**
 * Group a text node's characters into the visual lines the browser actually
 * laid them out on, by measuring each character's own rect. Returns null if
 * nothing measurable was found, so the caller can fall back.
 */
function measureWrappedLines(
  node: Node,
  raw: string,
): { text: string; rect: LineRect }[] | null {
  const range = document.createRange();
  const lines: { text: string; rect: LineRect }[] = [];
  let start = 0;
  let rect: LineRect | null = null;
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    // Collapsed whitespace measures as nothing; it belongs to whichever line
    // the surrounding characters are on, and gets trimmed either way.
    if (r.height === 0) continue;

    // A new line box starts wherever a character's top jumps. The tolerance
    // absorbs sub-pixel differences between glyphs on the same line.
    if (top === null || Math.abs(r.top - top) > 1) {
      if (rect) lines.push({ text: raw.slice(start, i), rect });
      start = i;
      top = r.top;
  ...
  if (rect) lines.push({ text: raw.slice(start), rect });
...
function paintFallbackText(
  ...
  function drawLine(line: string, rect: LineRect) {
    if (!line.trim()) return;
    if (
      !(
    ...
    const x = rect.left - rootRect.left + width * anchor;
    const metrics = ctx.measureText(line);
    const ascent = metrics.fontBoundingBoxAscent ?? 0;
    const descent = metrics.fontBoundingBoxDescent ?? 0;
    const y =
      ascent > 0
        ? rect.top - rootRect.top + (height - ascent - descent) / 2 + ascent
        : rect.bottom - rootRect.top - height * 0.2;
    ctx.fillText(line, x, y, Math.max(width, 1));
  }
  ...
  for (const node of textNodes) {
    ...
    /*
     * LOCAL FIX to the vendored fallback (see the header of this file).
     *
     * Upstream draws a wrapped text node by splitting its string across the
     * node's line boxes in proportion to each box's width -- which assumes
     * every character is the same width. In a proportional face that lands
     * mid-word: a three-line serif headline rendered as
     * "into unnecessaril / y deep problems."
     *
     * Instead, ask the browser where each character actually went and group
     * characters into lines by the top of their own rect. That reproduces the
     * real line breaking exactly. It costs one Range per character, so it is
     * only worth it for short runs -- long paragraphs fall through to the
     * original proportional path, where a mis-split is far less visible.
     */
    if (!preservesNewlines && raw.length > 0 && raw.length <= PER_CHAR_SPLIT_LIMIT) {
      const lines = measureWrappedLines(node, raw);
      if (lines) {
        for (const line of lines) {
          drawLine(transform(line.text.replace(/\s+/g, ' ').trim()), line.rect);
        }
        continue;
      }
    }
    ...
    if (preservesNewlines) {
      ...
      for (const part of raw.split('\n')) {
        ...
        const text = transform(preservesSpaces ? part : part.replace(/\s+/g, ' ').trim());
...
function initializeAsciiSweep(
  ...
  const firstCtx = slots[0].source.getContext('2d') as ElementImageContext | null;
  ...
  const states: SlotState[] = slots.map((slot, index) => ({
    ...
    ctx: index === 0 ? firstCtx : (slot.source.getContext('2d') as ElementImageContext | null),
  ...
  function syncCanvasSize(): boolean {
    ...
    contentMaxX = Math.min(
      1,
      Math.max(0.05, states[0].content.clientWidth / Math.max(output.clientWidth, 1)),
    );
    const dpr = Math.min(window.devicePixelRatio || 1, 2);
  ...
  function resolveGlyphs(): number {
    const ramp =
      config.glyphs.length > 1 ? config.glyphs : (CHARSETS[config.charset] ?? CHARSETS.ascii);
  ...
  if (!htmlInCanvas) {
    for (const state of states) {
      ...
      state.content.addEventListener('loadeddata', onFallbackVisualChange, true);
      ...
      state.content.addEventListener('transitionend', onFallbackVisualChange, true);
      state.content.addEventListener('animationend', onFallbackVisualChange, true);
    }
    ...
    destroy() {
      ...
      if (!htmlInCanvas) {
        for (const state of states) {
          ...
          state.content.removeEventListener('loadeddata', onFallbackVisualChange, true);
          state.content.removeEventListener('input', onFallbackVisualChange, true);
          state.content.removeEventListener('change', onFallbackVisualChange, true);
          state.content.removeEventListener('transitionend', onFallbackVisualChange, true);
          state.content.removeEventListener('animationend', onFallbackVisualChange, true);
        }
